...
from utilities.stats.file_io import (
    save_best_ind_to_file,
    save_first_front_to_file,
    save_stats_headers,
    save_stats_to_file,
)
from utilities.stats.save_plots import save_pareto_fitness_plot, save_plot_from_data
    ...
    if params["SAVE_PLOTS"] and not params["DEBUG"]:
        ...
        if params["VERBOSE"] or end:
            ...
            for o, ff in enumerate(params["FITNESS_FUNCTION"].fitness_functions):
                ...
                if ff.label is ff.__class__.__name__:
                    plotname = ff.label + str(o)
                    print(plotname, "  -- ", o)
                else:
                    plotname = ff.label
